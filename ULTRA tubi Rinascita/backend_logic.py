...
def _order_priority_thresholds(config=None):
    ...
    thresholds = {}
    for priority, key, default in (
        ("bassa", "order_priority_low_quantity", 1),
        ("media", "order_priority_medium_quantity", 3),
        ("alta", "order_priority_high_quantity", 5),
    ):
        try:
            thresholds[priority] = max(0, int(config.get(key, default)))
        except (TypeError, ValueError):
            thresholds[priority] = default
    return thresholds


def _order_include_low_priority(config=None):
    config = config or load_config(silent=True) or {}
    value = config.get("order_include_low_priority", True)
    if isinstance(value, str):
        return value.strip().lower() in {"1", "true", "yes", "si", "on"}
    return bool(value)


def get_tube_order_priorities(da_fare_path=None):
    ensure_result = ensure_tube_database(da_fare_path)
    if ensure_result.get("status") != "success":
        return {"status": "error", "message": ensure_result.get("message", "Database Tubi non disponibile.")}
    return {
...
def set_tube_order_priorities(payload, da_fare_path=None):
    ensure_result = ensure_tube_database(da_fare_path)
    if ensure_result.get("status") != "success":
        return {"status": "error", "message": ensure_result.get("message", "Database Tubi non disponibile.")}
...
def _order_decimal_text(value):
    return tube_database.normalize_decimal_text(value).replace(".", ",")


def _format_order_tube_line(row):
    misura = tube_database.normalize_misura(row.get("misura"))
    misura = "x".join(_order_decimal_text(part) for part in misura.split("x"))
    spessore = _order_decimal_text(row.get("spessore"))
    forma = str(row.get("forma") or tube_database.classify_tube(misura)[0]).lower()
    if forma == "tondo":
        shape_label = "TUBO TONDO"
        size_label = f"\u00d8{misura}x{spessore}"
    elif forma == "quadrato":
        shape_label = "TUBOLARE QUADRO"
        size_label = f"{misura}x{spessore}"
    else:
        shape_label = "TUBOLARE RETTANGOLARE"
        size_label = f"{misura}x{spessore}"
    return " ".join([
        str(row.get("codice") or "").strip().upper(),
        "AISI",
        str(row.get("materiale") or "304").strip().upper(),
        shape_label,
        size_label,
    ])


def generate_tube_order_txt(da_fare_path=None):
    ensure_result = ensure_tube_database(da_fare_path)
    if ensure_result.get("status") != "success":
        return {"status": "error", "message": ensure_result.get("message", "Database Tubi non disponibile.")}
    ...
    try:
        ...
        thresholds = _order_priority_thresholds(config)
        ...
        for row in tube_database.get_order_priority_rows(VARIABLES_ROOT):
            ...
            if priority == "bassa" and not include_low_priority:
                continue
            if priority and quantity <= thresholds[priority]:
                eligible.append(row)

        eligible.sort(key=lambda row: (
            priority_rank.get(row.get("priorita"), 9),
            row.get("forma", ""),
            str(row.get("misura", "")),
            tube_database._sort_number(row.get("spessore")),
            row.get("materiale", ""),
            row.get("codice", ""),
        ))
        ...
        seen_lines = set()
        for row in eligible:
            line = _format_order_tube_line(row)
        ...
        if not lines:
            return {
                "status": "empty",
                "message": "Nessun tubo codificato ha raggiunto la soglia di riordino.",
                "rows_count": 0,
                "thresholds": thresholds,
                "include_low_priority": include_low_priority,
            }

        output_dir = _get_docx_output_dir_fallback(da_fare_path, "order_txt_output_dir")
        filename = f"Ordine Tubi {datetime.now().strftime('%Y-%m-%d %H%M%S')}.txt"
