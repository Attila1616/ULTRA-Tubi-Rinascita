...
ORDER_PRIORITY_DEFAULT = "media"
ORDER_PRIORITIES = ("bassa", "media", "alta")
...
def normalize_order_priority(value, default=""):
    priority = str(value or "").strip().lower()
    return priority if priority in ORDER_PRIORITIES else default
...
def _normalize_code_catalog(catalog):
    ...
    for entry in (catalog or {}).get("entries", []):
        ...
        priority = normalize_order_priority(clean_entry.get("priorita"))
        if priority:
            clean_entry["priorita"] = priority
        else:
            clean_entry.pop("priorita", None)
...
def import_codes_from_excel(project_root, source_dir=None):
    ...
    for old_entry in old_catalog.get("entries", []):
        old_key = identity_key_from_values(
            old_entry.get("misura"),
            old_entry.get("spessore"),
            old_entry.get("materiale"),
            old_entry.get("finitura"),
        )
        old_priority = normalize_order_priority(old_entry.get("priorita"))
        if old_priority:
            old_priorities[old_key] = old_priority
    ...
    for key, priority in old_priorities.items():
        if key in entries:
            entries[key]["priorita"] = priority
    ...
    for entry in load_code_catalog(project_root).get("entries", []):
        ...
        code_map[key] = {
            **entry,
            ...
            "code": code,
            "hasCode": bool(code),
            "priorita": normalize_order_priority(entry.get("priorita")),
        }
    return code_map
...
def summarize_inventory(project_root, filters=None):
    ...
    for row in grouped.values():
        ...
        row["priorita"] = normalize_order_priority(
            code_info.get("priorita"),
            ORDER_PRIORITY_DEFAULT if row["hasCode"] else "",
        )
            ...
            for tube in ripiano.get("tubi", []):
                ...
                enriched = {
                    **tube,
                    ...
                    "priorita": normalize_order_priority(
                        code_info.get("priorita"),
                        ORDER_PRIORITY_DEFAULT if code_info.get("hasCode") else "",
                    ),
                }
...
def ensure_order_priorities_for_inventory(project_root):
    """Track every coded tube currently stocked, defaulting it to medium priority."""
    ensure_code_catalog(project_root)
    stocked_keys = {
        key for key, quantity in _inventory_totals(project_root).items()
    ...
    for entry in catalog.get("entries", []):
        if not _entry_code(entry):
            continue
        key = identity_key_from_values(
            entry.get("misura"),
            entry.get("spessore"),
            entry.get("materiale"),
            entry.get("finitura"),
        )
        priority = normalize_order_priority(entry.get("priorita"))
        if key in stocked_keys and not priority:
            entry["priorita"] = ORDER_PRIORITY_DEFAULT
            priority = ORDER_PRIORITY_DEFAULT
            changed = True
        if priority:
            tracked += 1
    ...
    if changed:
        save_code_catalog(project_root, catalog)
    return {"status": "success", "changed": changed, "tracked": tracked}


def get_order_priority_rows(project_root):
    ensure_order_priorities_for_inventory(project_root)
    totals = _inventory_totals(project_root)
    rows = []

    for entry in load_code_catalog(project_root).get("entries", []):
        code = _entry_code(entry)
        priority = normalize_order_priority(entry.get("priorita"))
        if not code or not priority:
            continue
        key = identity_key_from_values(
            entry.get("misura"),
            entry.get("spessore"),
        ...
        rows.append({
            **identity,
            "codice": code,
            "priorita": priority,
            "quantitaTotale": int(totals.get(key, 0) or 0),
            "presenteInInventario": key in totals,
        })

    rows.sort(key=lambda row: (
        row.get("forma", ""),
        row.get("misura", ""),
        _sort_number(row.get("spessore")),
        row.get("materiale", ""),
        row.get("finitura", ""),
    ))
    return rows
...
def set_order_priorities(project_root, payload):
    payload = payload or {}
    updates = {}
    for item in payload.get("items", []):
        ...
        priority = normalize_order_priority(item.get("priorita"))
        if key and priority:
            updates[key] = priority

    if not updates:
        return {"status": "error", "message": "Nessuna priorita valida da salvare."}
    ...
    for entry in catalog.get("entries", []):
        ...
        if key not in updates:
            continue
        if normalize_order_priority(entry.get("priorita")) != updates[key]:
            entry["priorita"] = updates[key]
            updated_count += 1
    ...
    return {
        "status": "success",
        "message": "Priorita ordini aggiornate.",
        "updated": updated_count,
        "rows": get_order_priority_rows(project_root),
    }
